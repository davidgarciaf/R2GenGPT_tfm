...
# HF_TOKEN = os.getenv('HF_TOKEN_LLAMA2')
HF_TOKEN = os.getenv('HF_TOKEN')
if not HF_TOKEN:
    raise ValueError("El modelo no encuentra el token de Hugging Face. Asegúrate de que el archivo .hf_token.env esté correctamente configurado.")
else:
...
class R2GenGPT(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        # Congelamiento del encoder si fuera neceario
        elif args.freeze_vm:
        ...
        # Carga LLM preentrenado para la geeración de texto
        # self.llama_tokenizer = LlamaTokenizer.from_pretrained(args.llama_model, use_fast=False)
        # self.llama_tokenizer = LlamaTokenizer.from_pretrained(args.llama_model, use_fast=False, trust_remote_code=True, token=True) #Llama3 o3
        self.llama_tokenizer = AutoTokenizer.from_pretrained(
            args.llama_model,
            trust_remote_code=True,       # carga el chat-template oficial
            token=True)                   # lee el $HF_TOKEN
        ...
        # Proyección para mapear las imágenes al tamaño oculto de LlaMA
    ...
    def score(self, ref, hypo):
        """
        Función para el cálculo de score
        BLEU: similitud de ngramas
    ...
    def encode_img(self, images):
        # Extrae las features de las imágenes usando el Swin Transformers
        image_embeds = []
        for image in images:
            device = image.device
            # Usa solo el pooler_output
    ...
    # Perpara el formato para el input de LLaMA insertando los embeddings de imagen en un prompt estructurado
    ...
    def forward(self, samples):
        image = samples["image"]
        # Genera los features de las imágenes
        img_embeds, atts_img = self.encode_img(image)
        # Aplica normalización a la capa
        img_embeds = self.layer_norm(img_embeds)
        
        # Wrap de los images embeddings en el prompt de texto
        img_embeds, atts_img = self.prompt_wrap(img_embeds, atts_img)
        # Tokeniza el texto de salida esperado
        self.llama_tokenizer.padding_side = "right"
        text = [t + self.end_sym for t in samples["input_text"]]

        # Crea un tensor input combinando begining of sequence token, image embeddings y text embeddings.
    ...
    def validation_step(self, samples, batch_idx):
        # Similar a forward pero en lugar de calcular la pérdida hace lo siguiente:
        # Genera el texto output, decodifica los tokens en texto legible y almacena la hipotesis generada y el texto de referencia
        self.llama_tokenizer.padding_side = "right"
        to_regress_tokens = self.llama_tokenizer(
            samples['input_text'],
            return_tensors="pt",
